...
object Config {
  ...
  def getAllowedAccountIds(config: Configuration): Option[Seq[String]] = config.getOptional[Seq[String]]("alert.allowedAccountIds")

  def getConfigProperties(config: Configuration)(implicit ec: ExecutionContext): Attempt[UnrecognisedJobConfigProperties] = {
    for {
    ...
    } yield UnrecognisedJobConfigProperties(accounts, key, bucket, securityAccount)
  }
